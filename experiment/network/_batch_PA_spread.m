...
scatter3(PCsomata(:,1),PCsomata(:,2), PCsomata(:,3), 80, [0,0,1], 'fill', 'o');hold on;
...
clcn = AllConnMat_str .* AllConnMat_str';
clcn = clcn(1:pc,1:pc);
for cl = 1:NC_str(Sid)
    clidx = find(labels_str==cl);
    scatter3(PCsomata(clidx,1),PCsomata(clidx,2), PCsomata(clidx,3), 80, rand(1,3), 'fill', 'o');hold on;
%     clcmb = combntns(clidx,2) ;
    % check connectivity also:
    [clcn_r,clcn_c] = find(clcn(clidx,clidx));
    for i = 1:numel(clcn_r)
        plot3([PCsomata(clidx(clcmb(1)),1),PCsomata(clidx(clcmb(1)),1)],PCsomata(clidx(clcmb(1)),2), PCsomata(clidx(clcmb(1)),3));hold on;
    end
end
axis equal; hold on;

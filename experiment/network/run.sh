...
stimnoise="0.0"
inhibias="3.5"
excitbias="3.5" # must make it 4 and see the same behaviour
gababfactor="2"
pv2pc="4"
pc2pc="4" # this is now depricated.
...
for loccl in $(seq 3 3); do
for cluster in $(seq 0 9); do
#for inhibias  in $(seq 9 0.5 20); do
...
#	cluster="${run}"
	if [ "$exp" == "1" ]; then
		...
		jobname="custompvsn1LOCCL${loccl}SF${stimfreq}SN${stimnoise}_pc2pc${pc2pc}ctrI50_EB$(printf '%.3f' $excitbias)_IB$(printf '%.3f' $inhibias)_ST${stimmagnitude}_GBF$(printf '%.3f' $gababfactor)_NMDAb$(printf '%.3f' $nmdabias)_Ab$(printf '%.3f' $ampabias)_${exp_str}s${tstop_sec}c${cluster}_SN${sn}_r"
	else
		exp_str="R"

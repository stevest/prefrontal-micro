...
pathprefix = '\\139.91.162.90\cluster\stefanos\Documents\Glia\TEST';
...
for ru = 1:run.nruns
    for c=1:run.nPC+run.nPV
        ...
        rate = zeros(1,tstop);
        tevents = cell(numberOfSyns,1);
        for kk=1:numberOfSyns % input neurons BG
            tmp = zeros(1,tstop);
            rndshuffle = rand(1,events)*((tstop/events)*1); %the factor changes Vm variability!
            rndshuffle = rndshuffle - ((tstop/events)*1)/2;
            rndshuffle = round(linspace(10,tstop,events) + rndshuffle);
            rndshuffle(rndshuffle<=0) = [];
            tmp(rndshuffle) = 10;
            rate(1:tstop) = conv(tmp(1,1:tstop),kernel,'same');
            tevents{kk} = nonhomogenouspoissrnd(tstop,0.12,rate);
        end

...
st_str = cell(run.nPC+run.nPV,run.nruns);
for stc=clusters2load
%     RUNS{1,stc} = cell(run.nPC+run.nPV,run.nruns);
    for ru = runs2load
        fprintf('Run is: %d, of cluster %d\n',ru,stc);
        run.path = sprintf('Ss20c%dr%d',stc-1,ru-1);
        for pc=1:run.nPC
            fprintf('%d,',pc);
            if (run.ISBINARY)
                try
                    filename = sprintf('%s%s/%d_%d_%d.bin',pathprefix,run.path,stc-1,pc-1,ru-1);
                    if( exist(filename,'file') )
                        tmp = ncell(nrn_vread(filename,'n'),10);
                        tmp = tmp(1:10:end)
                        % Use advanced_spike_count because ncell.spike_count is broken:
                        tic;
                        [~, st_str{pc,ru}] = advanced_spike_count(tmp.mv,-20,0);
                        fprintf('Spike count took: %f \n',toc);
%                         RUNS{1,stc}{pc,ru} = ncell(nrn_vread(filename,'n'),10);
%                         RUNS{1,stc}{pc,ru}.clusterID = run.labels_str(pc,1);
                        % Use advanced_spike_count because ncell.spike_count is broken:
%                         [~,RUNS{1,stc}{pc,ru}.spikes] = spike_count(RUNS{1,stc}{pc,ru}.mv);
%                         RUNS{1,stc}{pc,ru}.spikes = single(RUNS{1,stc}{pc,ru}.spikes);
%                         PCcells_str{pc,ru}.mv = [];
                    else
                        st_str{pc,ru} = NaN;
                        fprintf('File nonexistent!');
                    end
                catch err
                    warning('on','verbose')
                    warning(err.message)
...
%         for pv = nPC+1:nPC+nPV
%             fprintf('%d,',pv);
%             filename = sprintf('%s%s/%d_%d_%d.bin',pathprefix,run.path,stc-1,pv-1,ru-1);
%             if( exist(filename,'file') )
%                 tmp = ncell(nrn_vread(filename,'n'),10);
%                 [~, st_str{pv,ru}] = spike_count(tmp.mv);
% %                 RUNS{1,stc}{pv,ru} = ncell(nrn_vread(filename,'n'),10);
% %                 [~,RUNS{1,stc}{pv,ru}.spikes] = spike_count(RUNS{1,stc}{pv,ru}.mv);
% %                 RUNS_str{1,stc}{pv,ru}.mv = [];
%             else
%                 fprintf('File nonexistent!');
%             end
%         end
...
save('\\139.91.162.90\cluster\stefanos\Documents\Glia\RUNS_str_STC0_20S.mat','st_str','-v7.3');
%% Visualize RUNS as whole network
for ru=1:1
IFRArray = zeros(run.nPC,tstop);
for k=1:run.nPC
    spikes = RUNS{1,stc}{k,ru}.spikes;
    if ~isempty(spikes)
        IFRArray(k,:) = instantfr(spikes,linspace(0,tstop,tstop));
    end
end

...
runs = runParams.runrange;
if isempty(loadParams)
    for ri = 1:length(runs)
        pathto = fullfile(osDrive(),'Documents','Glia',sprintf(experimentDirStr, experimentDirArg{ri}{:}));
        batch = load_raw_batch(pathto);
%         if ~isempty(batch)
            batch_all(1,ri) = {batch(1:933,:)};
%         else
%             warning('Empty data loaded!!!');
%         end
        batch = batch_all{1,ri};
        save(fullfile(osDrive(),'Documents','Glia',sprintf(runParams.experimentFileStr, runParams.experimentFileArg{ri}{:})),'batch','-v7.3');
    end
else
...
%         specificBatch = specificBatch_all{1,ru};
        filename = fullfile(osDrive(),'Documents','Glia',sprintf(loadParams.specificsFileStr, loadParams.specificsFileArg{ri}{:}));
        disp(sprintf('Saving file: %s.',filename));
        save(filename,'specificBatch','-v7.3');
    end
end

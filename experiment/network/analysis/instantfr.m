...
if nargin<2
    ...
    tt0 = [tt(1) tt0 tt(end)];
    % Interpolate same values so the interp1 does not complain:
    [tt1,~,idx] = unique(tt0,'stable');
    ifr1 = accumarray(idx,ifr0,[],@mean); %You could take something other than the mean.
    
    ifr = interp1(tt1,ifr1,tt,'linear');
    if size(tt,1)~=1
        ifr=ifr';
    end
end

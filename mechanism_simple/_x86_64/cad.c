...
extern Memb_func* memb_func;
 ...
 "setdata_cadyn_out", _hoc_setdata,
 0, 0
};
 /* declare global and static user variables */
#define cainf cainf_cadyn_out
 double cainf = 0.0001;
#define depth depth_cadyn_out
 double depth = 0.1;
#define taur taur_cadyn_out
 double taur = 200;
 ...
 "depth_cadyn_out", &depth_cadyn_out,
 "taur_cadyn_out", &taur_cadyn_out,
 "cainf_cadyn_out", &cainf_cadyn_out,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 cadyn_out /cluster/stefanos/Documents/GitHub/prefrontal-micro/mechanism_simple/x86_64/cad.mod\n");
...
/*CVODE*/
 ...
 static int state (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   ...
   if ( drive_channel <= 0. ) {
    ...
    ca = ca + (1. - exp(dt*((( ( ( - 1.0 ) ) ) / taur)*(7.0))))*(- ( ( drive_channel ) / 18.0 + (( ( cainf ) ) / taur)*(7.0) ) / ( (( ( ( - 1.0) ) ) / taur)*(7.0) ) - ca) ;
   cai = ca ;
   }
  return 0;
}

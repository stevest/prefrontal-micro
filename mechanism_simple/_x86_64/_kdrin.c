...
extern Memb_func* memb_func;
 ...
 "setdata_kdrIN_out", _hoc_setdata,
 "alf_kdrIN_out", _hoc_alf,
 "bet_kdrIN_out", _hoc_bet,
 "rate_kdrIN_out", _hoc_rate,
 0, 0
};
#define alf alf_kdrIN_out
#define bet bet_kdrIN_out
 extern double alf( _threadargsprotocomma_ double );
 extern double bet( _threadargsprotocomma_ double );
 /* declare global and static user variables */
 /* some parameters have upper and lower limits */
 static HocParmLimits _hoc_parm_limits[] = {
 "gkdrbar_kdrIN_out", 0, 1e+09,
 0,0,0
};
 static HocParmUnits _hoc_parm_units[] = {
 "gkdrbar_kdrIN_out", "mho/cm2",
 "ik_kdrIN_out", "mA/cm2",
 "gk_kdrIN_out", "mho/cm2",
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 5);
 	...
 	ivoc_help("help ?1 kdrIN_out /cluster/stefanos/Documents/GitHub/prefrontal-micro/mechanism_simple/x86_64/_kdrin.mod\n");
...
double alf ( _threadargsprotocomma_ double _lv ) {
 ...
 _lva = _lv - 3.0 ;
   if ( fabs ( _lva ) < 1e-04 ) {
     ...
     _lalf = ( - 0.018 * ( _lv - 3.0 ) ) / ( - 1.0 + exp ( - ( ( _lv - 3.0 ) / 25.0 ) ) ) ;
     }
...
double bet ( _threadargsprotocomma_ double _lv ) {
 ...
 _lvb = _lv - 13.0 ;
   if ( fabs ( _lvb ) < 1e-04 ) {
     ...
     _lbet = ( 0.0054 * ( _lv - 13.0 ) ) / ( - 1.0 + exp ( ( _lv - 13.0 ) / 12.0 ) ) ;
     }

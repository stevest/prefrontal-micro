...
extern Memb_func* memb_func;
 ...
 "setdata_NafIN_out", _hoc_setdata,
 "hbet_NafIN_out", _hoc_hbet,
 "half_NafIN_out", _hoc_half,
 "mbet_NafIN_out", _hoc_mbet,
 "malf_NafIN_out", _hoc_malf,
 0, 0
};
#define hbet hbet_NafIN_out
#define half half_NafIN_out
#define mbet mbet_NafIN_out
#define malf malf_NafIN_out
 ...
 "gNafbar_NafIN_out", 0, 1e+09,
 0,0,0
};
 static HocParmUnits _hoc_parm_units[] = {
 "gNafbar_NafIN_out", "mho/cm2",
 "gna_NafIN_out", "mho/cm2",
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 NafIN_out /cluster/stefanos/Documents/GitHub/prefrontal-micro/mechanism_simple/x86_64/NafIND.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   Dh = ( 1.0 - h ) * half ( _threadargscomma_ v ) - h * hbet ( _threadargscomma_ v ) ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 Dm = Dm  / (1. - dt*( (( ( - 1.0 ) ))*(malf ( _threadargscomma_ v )) - (1.0)*(mbet ( _threadargscomma_ v )) )) ;
 Dh = Dh  / (1. - dt*( (( ( - 1.0 ) ))*(half ( _threadargscomma_ v )) - (1.0)*(hbet ( _threadargscomma_ v )) )) ;
 return 0;
}
 /*END CVODE*/
...
double half ( _threadargsprotocomma_ double _lv ) {
   double _lhalf;
 _lhalf = 0.098 * exp ( - ( _lv + 53.1 ) / 20.0 ) * 2.0 ;
...
double hbet ( _threadargsprotocomma_ double _lv ) {
   double _lhbet;
 _lhbet = 1.4 / ( 1.0 + exp ( - ( _lv + 23.1 ) / 10.0 ) ) * 2.0 ;

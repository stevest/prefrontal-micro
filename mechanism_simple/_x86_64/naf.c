...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 Naf /cluster/stefanos/Documents/GitHub/prefrontal-micro/mechanism_simple/x86_64/naf.mod\n");
...
double malf ( _threadargsprotocomma_ double _lv ) {
   double _lmalf;
 if ( fabs ( ( _lv + 28.0 ) ) < 1e-04 ) {
     _lmalf = - 0.2816 * ( - 9.3 + ( _lv + 28.0 ) * 0.5 ) ;
     }
   else {
...
double mbet ( _threadargsprotocomma_ double _lv ) {
   double _lmbet;
 if ( fabs ( ( _lv + 1.0 ) ) < 1e-04 ) {
     _lmbet = 0.2464 * ( 6.0 + ( _lv + 1.0 ) * 0.5 ) ;
     }
   else {
...
double half ( _threadargsprotocomma_ double _lv ) {
   double _lhalf;
 if ( fabs ( ( _lv + 43.1 ) ) < 1e-04 ) {
     _lhalf = 0.098 * ( 20.0 + ( _lv + 43.1 ) * 0.5 ) ;
     }
   else {
...
double hbet ( _threadargsprotocomma_ double _lv ) {
   double _lhbet;
 if ( fabs ( ( _lv + 13.1 ) ) < 1e-04 ) {
     _lhbet = 1.4 * ( 10.0 + ( _lv + 13.1 ) * 0.5 ) ;
     }
   else {
